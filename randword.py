...
def apply_affixes(base_word, affix_rules):
    ...
    Returns:
    List object containing all words formed by applying affixes to the base word.
    """
    complete_words = []
    partial_words = []
    ...
    complete_words.extend([apply_suffix(base_word, affix_rule[3:]) for affix_rule in affix_rules
                           if affix_rule[0] == "SFX" and not affix_rule[1]])
    complete_words.extend([apply_prefix(base_word, affix_rule[3:]) for affix_rule in affix_rules
                           if affix_rule[0] == "PFX" and not affix_rule[1]])
    partial_words.extend([apply_suffix(base_word, affix_rule[3:]) for affix_rule in affix_rules
                           if affix_rule[0] == "SFX" and affix_rule[1]])
    for partial_word in partial_words[:]:
        partial_words.extend([apply_prefix(partial_word, affix_rule[3:]) for affix_rule in affix_rules
...
def apply_prefix(base_word, prefix_rules):
    ...
    for prefix_rule in prefix_rules:
        ...
        part_word = base_word[len(delete_chars):] if len(delete_chars) > 0 else base_word
        break
    else:
        return base_word
...
def get_affix_rules(affix_file):
    """
    Read affix rules from a specified file and return these in a structured format
    ...
    Dict object with the following structure:
        ...
        Value:  List object containing the following objects:
                    [0]: String object containing the value "PFX" or "SFX" to indicate the affix type
                    [1]: Boolean object indicating whether this affix can be combined with other affixes 
                         (only 1 prefix and 1 suffix can be added to a base word)
                    [2]: Integer object indicating how many match rules exist for this affix
                    [3]: 1 or more tuple objects containing the match rules, as follows:
                         [0]: Any letter(s) to be removed from the word
                              (Remove from start of word for prefixes and from end for suffixes,
                              although prefixes do not generally require any removal)
                         [1]: Letters to be added to the word for this rule
                              (Add to start for prefix, add to end for suffix)
                         [2]: Regex expression to be matched for this rule
    """
    rule_dict = {}
    file_open = False
    try:
            ...
            for line in f:
                if not re.match("^[PS]FX\s", line):
                    ...
                    if rule_count == 0:
                        ...
                        rule_id = rule_data[1]
                        can_combine = True if rule_data[2] == "Y" else False
                        rule_count = int(rule_data[3])
                        rule_dict[rule_id] = [rule_type, can_combine, rule_count]
                    else:
                        rule_id = rule_data[1]
                        rule_data[2] = '' if rule_data[2] == "0" else rule_data[2]
                        rule_data[4] = "^" + rule_data[4][:] if rule_type == "PFX" else rule_data[4][:] + "$"
                        rule_dict[rule_id].append(tuple(rule_data[2:]))
                        rule_count -= 1
                
    except OSError:
        error_type = "reading" if file_open else "opening"
        print("Error while " + error_type + " affix file " + affix_file, file=sys.stderr)
...
def get_words(dictionary_file, num_words):
    ...
    Returns:
    A list of tuples, each of which is obtained by applying the
    str.partition("/") function to each line read from the dictionary file
    """
    words = []
        ...
        try:
            with open(dictionary_file, "rb") as f:
                ...
                while len(words) < num_words:
                    word_binary = b''
                    while word_binary == b'':
                        random_pos = int(random.random() * os.path.getsize(dictionary_file))
                        mm.seek(random_pos)
                        # Word at current position may only be partial, so discard
                        mm.readline()
            ...
            print("Error opening " + dictionary_file + " for reading", file=sys.stderr)
            return None
    
        except ValueError:
            mm.close()
            print("Seek error at position " + random_pos + " in dictionary file " + dictionary_file, file=sys.stderr)
